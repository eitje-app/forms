...
const Label = (props) => {
	...
	return (
		<>
			<Text popoutTitle={popoutTitle} darkGrey fontSize={12}>
				{label} {required && "*"}
			</Text>
			{extraLabel && (
				<Text darkGrey fontSize={12}>
					{extraLabel}
				</Text>
			)}
		</>
...
const buildDecoration = (props) => {
	...
	let val = props.hasOwnProperty(decorationType)
		? props[decorationType]
		: decorationDefaults[decorationType];
...
const makeTranslation = (props) => {
	...
	allKeys = [
		...allKeys,
		`form.defaults.fields.${field}.${decorationType}`,
		`records.default.fields.${field}`,
		lastOption,
	].filter(Boolean);
